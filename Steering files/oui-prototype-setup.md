...
When scaffolding a new OUI prototype React app, ALWAYS use the folder name `oui-prototype`:
...
4. Configure `vite.config.ts` with the datemath alias:
   ```ts
...
5. Add the `global` polyfill in `index.html` before the module script tag
   (OUI dependencies expect Node's `global` variable):
   ```html
...
## 2. Theme System (REQUIRED for every prototype)

OUI ships with 6 theme CSS files. Every prototype MUST include a ThemeProvider and ThemePicker.
...
### ThemePicker Pattern (REQUIRED)

Create `src/components/ThemePicker.tsx` that:
- Renders an `OuiButtonIcon` with `display="empty"` and `size="s"`
- Uses `iconType="moon"` for light themes, `iconType="invert"` for dark themes
  (OUI does NOT have a `sun` icon — use `invert` instead)
- On click, shows a dropdown panel (use `OuiPanel` with absolute positioning, NOT `OuiPopover`
  which has portal rendering issues in React 18)
...
## 5. Icon Cache (REQUIRED for React 18)

OUI's `OuiIcon` is a class component that uses dynamic `import()` to load icon SVGs at runtime.
This triggers a `setState` before mount in React 18 strict mode, causing icons to render as
empty gray boxes.
...
**IMPORTANT:** OUI does NOT have these commonly assumed icons:
- No `sun` icon — use `invert` for light mode toggle
- No `dark`/`light` icons — use `moon`/`invert`
- No `home` icon exists as `home` — it does exist, use `home`
...
- NOTE: `OuiPopover` has portal rendering issues in React 18 with Vite. For dropdown menus,
  use `OuiPanel` with absolute positioning and a click-outside handler instead.

---

## 9. Design Token & Theming Standards

- Use OUI's built-in color variables and spacing. Do NOT hardcode hex colors or pixel values.
- Use CSS custom properties from the active theme: `var(--ouiColorPrimary)`,
  `var(--ouiColorLightestShade)`, etc.
- Stick to OUI's 4px/8px spacing grid. Common spacing values: 4, 8, 12, 16, 24, 32, 48.
- For typography, use `OuiText`, `OuiTitle`, and `OuiCode` components rather than raw HTML tags.
- When extending the theme, place overrides in `src/tokens/` and document what you changed.
...
- Every form input must have an associated `<label>` or use OUI's `OuiFormRow`.
...
- OUI Living Style Guide: https://oui.opensearch.org/
